...
- **Product cards** open the product page; "Featured" products show the "Най-популярен" badge.
- **Product page** (`themes/reklamo/woocommerce/content-single-product.php`) follows the client's design: gallery, category eyebrow, the final price in a circle, "Какво включва пакетът?", a card listing what is included with the single call to action, the four ordering steps, the tabs and the help card. There is no add-to-cart anywhere: the CTA goes to `/kachi-logo/?paket=<slug>`. The step strip reads the mockup deadline and deposit percentage from Settings → Reklamo, so changing a setting changes the page. **Tabs** use WooCommerce's own `woocommerce_product_tabs`: Описание comes from the product's description and Спецификации from the **Specifications** box in Products → Product data → Основни (`Reklamo_Product`, meta `_reklamo_specs`), while Брандиране, Доставка и срокове and ЧЗВ come from the pages that already carry that text. A tab disappears when it has nothing to show — an empty Specifications box, or a page still holding only the seed placeholder.
- **Catalogue**: four promo packages in Пакети, and six Бизнес календар START 1–6 in Календари — one product per quantity tier (20/50/100/200/300/500 броя), priced and photographed from the client's artwork, with the supplied paper and print specifications as the description. Products are created only when their SKU is missing, so names, prices and texts belong to the dashboard afterwards. Bulgarian names would become percent-encoded URLs, so `ensure_product` takes an explicit latin slug.
