...
- **Customer order page**: every customer email carries a passwordless link (`/moyata-porachka/?s=…&k=…`) to a view-only page with progress, mockup history, payments and a "send the email again" button. No accounts. The link expires together with the order's files under the retention setting.
