...
- `mariadb:11.8` already cached. All four image tags confirmed to resolve: `wordpress:7.1-php8.5-apache`, `wordpress:cli-2-php8.5`, `axllent/mailpit:v1.31`.
...
- cPanel + CloudLinux + Apache with `.htaccess`. PHP per account via *PHP Manager by SuperHosting*, ea-php52 through ea-php85 installed on saturn, web default **8.5** (the shell's bare `php` is 8.4, a different default). CLI binaries at `/opt/cpanel/ea-phpXX/root/usr/bin/php`. WordPress 7.1 and WooCommerce 11.1 fully support 8.5 (make.wordpress.org, May 2026 clarification). The local stack runs `php8.5` images to match.
- On saturn (checked over SSH): WP-CLI phar at `/usr/local/bin/wp-cli.phar` behind a bash launcher `/usr/local/bin/wp-cli` that runs it on the newest PHP present (a launcher is not a phar: feeding it to PHP prints its source, which is how the first install attempt failed), git 2.48, `uapi` present, `wget` works, `/usr/bin/curl` is root-only. All PHP extensions the plugin needs are loaded in ea-php84. `public_html` holds only the SuperHosting placeholder page. One mailbox exists (the cPanel user's own), no databases yet.
...
- Outbound 22/25/26/465 to external hosts blocked: GitHub via `ssh.github.com:443`, mail only through the account's own server (587 tls on `serverNN.superhosting.bg`; 465 is refused on saturn even from localhost; 25 plain as documented fallback). Exim sender-verifies the envelope address, so mail must leave as an existing mailbox.
...
- **Order deletion and anonymisation** (from implementing `docs/GDPR.md`): `woocommerce_before_delete_order` fires from both the HPOS and the legacy data store with `( $order_id, $order )`; trashing fires `woocommerce_before_trash_order` instead. HPOS deletion leaves `wp_comments` order notes behind, so the plugin deletes them itself. `WC_Order::save()` refreshes `date_modified`, so a retention test has to backdate `wp_wc_orders.date_updated_gmt` directly. `WC_Privacy_Erasers::remove_order_personal_data()` is the native anonymiser: it blanks billing/shipping/IP/user agent, sets `_anonymized = yes`, adds a note, and fires `woocommerce_privacy_before_remove_order_personal_data` first, which is where the plugin strips its own meta. `wc_get_orders()` accepts `meta_query` under HPOS, used to skip already anonymised orders. WooCommerce Order Attribution sets `sbjs_*` cookies on every visit; with it off the storefront sets no cookie at all (asserted in e2e).
- **`wp-sitemap.xml` 404s whenever `blog_public` is `0`**, which is how the dev stack is seeded, so sitemap work cannot be verified locally without flipping the option and flipping it back. WooCommerce's "Coming soon" mode is a separate thing again: with `woocommerce_coming_soon=yes` and `woocommerce_store_pages_only=yes` the store pages serve a placeholder to strangers while the rest of the site stays public, so an anonymous `curl` of a product page returns "Great things are on the horizon" and none of the real markup. `woocommerce_private_link=yes` hands out `?woo-share=…` links that bypass it and set a `woo-share` cookie for that visitor only.
- **The seed can only read files WP-CLI can see.** Locally WP-CLI runs in a container where just
  `./wp` and the two `wp-content` directories are mounted, so `scripts/` is invisible to it while
  the host sees it; on the server both are plain paths. Anything the seed imports therefore lives
  under the theme and is resolved *inside* WP-CLI with
...
| Question | Choice |
|---|---|
| Packages | **Real names + prices from `design/preview.webp`**: Red Business Pack 100 €, Office Starter Pack 119 €, Event Pack 149 €, Premium Pack 169 €. Prices are a field on each product — the owner edits them in Продукти → edit. Each package carries its own photo cut from the same mockup by `scripts/crop-previews.php` into `themes/reklamo/assets/img/packages/`, seeded only when the product has no image; they are stand-ins until the owner uploads real photography. |
...
| Service | Image | Port |
|---|---|---|
| `wp` | `wordpress:7.1-php8.5-apache` | 8080 |
| `db` | `mariadb:11.8` | — |
| `mail` | `axllent/mailpit:v1.31` | 8025 |
| `cli` | `wordpress:cli-2-php8.5` | — |
...
Decided with the owner: **no customer accounts**. Every email carries a permanent, passwordless link to the customer's own order page at `/porachka/?s=…&k=…` (`Reklamo_Tracking`, purpose `track` in the tokens table, URL kept in order meta so later emails can repeat it — a DB dump already exposes everything the page shows). The page is **view-only**: progress line (the six homepage steps, `Reklamo_Progress`), what happens next per status, every mockup revision with outcome and the customer's comments, payments (deposit/balance, bank details while due), invoice/delivery details, the logo. Approving and submitting details still need their own single-use tokens, so a forwarded email can read but never act. **Revised 2026-09-07 at the owner's request:** the customer approves or requests changes directly on this page (same `Reklamo_Approval::decide()` path as the emailed link; the emailed token is consumed so it cannot decide twice) and gets the details form via a button that issues a fresh details link. No "send the email again" on the customer side — mail failures are the owner's business and show up as order notes and admin notices. Trade-off accepted: whoever holds the tracking link can act on the order, the same trust already placed in the mailbox. The request form now lands on this page instead of WooCommerce's order-received. After completion the page becomes a read-only record; the retention sweep that deletes the order's files also expires the link, after which the page says so and asks to quote the order number. If repeat customers ever appear, WooCommerce's optional accounts can render the same page inside My Account. **Revised again 2026-09-07:** the order header and progress line (`templates/customer-head.php`) open every customer page, the emailed approval and details links included, and the highlighted step is the one *in progress* (a fresh request → Mockup, a sent mockup → Approval, an approved mockup → Deposit, deposit paid → Production), with everything before it ticked. The page after approving is titled "Deposit" (Аванс) and links back to the order page.

**Motion (2026-09-08).** Interaction-only micro-animations, decided with the owner: no entrance, hero or scroll animation. Theme tokens `--ease`, `--fast` (150 ms), `--base` (250 ms), `--focus` in `theme.css`; buttons lift 1 px with a soft gold shadow on hover, settle on press, and get a visible `:focus-visible` ring; nav underline grows from the left; package cards lift 3 px and their image scales 1.03; the mobile menu fades/slides in with staggered items (`visibility` + `opacity`, no `display` toggle); drop zone scales 1.01 with a solid border while dragging; file chip, errors and notices fade up; the chunked-upload bar has a sheen only while bytes are in flight. The standalone customer pages carry the same rules inline in `customer-style.php` (hex only, no external assets). Every file ends with a `prefers-reduced-motion: reduce` guard that zeroes durations.
...
- `cli` uses `wordpress:cli-2-php8.5`, run as `1000:1000`, sharing `./wp`, and is invoked with `run --rm` (never idles).
...
**Legal & content:** the full inventory, gap list and phased plan are in [`docs/GDPR.md`](GDPR.md): order deletion leaving files behind, Order Attribution cookies, no consent record, no anonymisation of closed orders, placeholder legal pages, missing trader identification and withdrawal waiver, shop notifications at a consumer mailbox.

**Theme / SEO / performance:** meta description + Open Graph + favicon; dequeue WooCommerce styles/scripts outside shop pages and `woocommerce_enable_ajax_add_to_cart no`; drop the gallery lightbox; preload the two Cyrillic woff2, hero `width/height/fetchpriority`; contrast (`--gold-dark → #8a6318`, `--muted → #6b6b6b`); header CTA hidden below 900px; nav Escape/outside-click close, `.rq-drop:focus-within`; textarea label in the full form; footer `h4 → h2`; `BreadcrumbList`; `woocommerce_before/after_main_content` in `archive-product.php`.

**Ops / CI:** HTTPS redirect + HSTS in `.htaccess`, `WP_DEBUG_LOG` path, backup cadence + restore drill, wp-login protection / `DISALLOW_FILE_MODS`, never *delete* the plugin, xmlrpc off. Done by the deployment scripts: `WP_AUTO_UPDATE_CORE minor`, `chmod 600 wp-config.php`, pins in `versions.env`, `update.sh` dumps the DB before version changes. GitHub Actions: lint + PHPUnit + E2E; mobile Playwright project; axe; rate-limit spec with limits on.

## Deployment scripts

`scripts/deploy.sh` (workstation) → `scripts/host/{bootstrap,install,update,check}.sh` (server). `install.sh` is idempotent and converges an existing account with its `.env`; `update.sh` fetches, switches the checkout under maintenance mode, converges WordPress/WooCommerce to `versions.env` (database dump first, `wp core update --force`, `wp plugin install --force`, `update-db`, `wp wc update`), flushes and runs `check.sh`, and re-runs `seed.sh` only with `--seed`. `check.sh` drops a one-off PHP probe into `public_html`, fetches it over HTTP and compares web PHP with `PHP_BIN`, prints the applied `upload_max_filesize` and friends, fetches the theme's `style.css` through the symlink and confirms the private directory is not addressable.

Decisions: theme and plugin are **symlinks** into the checkout (deploy = checkout move, rollback = older tag). `seed.sh` runs at install and only on `update --seed` afterwards (owner decision after go-live: dashboard edits to `opt` settings must survive releases); owner-editable content uses `opt_default` / create-if-missing so even a `--seed` run leaves it alone. `converge_versions` only ever **raises** WordPress and WooCommerce to the pins; a site ahead of `versions.env` is kept and reported, because `WP_AUTO_UPDATE_CORE minor` legitimately moves the server first. `WP_AUTO_UPDATE_CORE minor` on the host: security releases apply themselves, plugins and major versions stay pinned. The admin password is generated on first install and printed once, never stored.

**Simulation** (there is no SuperHosting account to test against): copy the repo to `/tmp/hosttest/reklamo` without `wp/`, `private/`, `vendor/`, write a server `.env` (`WP_PATH=/home/hosttest/public_html`, `PHP_BIN=/usr/local/bin/php`, `DB_HOST=db`, DB created in the dev MariaDB), and run inside `wordpress:cli-2-php8.5` on the compose network:

```bash
docker run --rm --user root --network reklamo_default -v /tmp/hosttest:/home/hosttest \
  -v "$PWD:$PWD:ro" -w /home/hosttest/reklamo wordpress:cli-2-php8.5 bash -c '
  apk add -q git; adduser -D -u 1000 -h /home/hosttest -s /bin/bash hosttest
  su hosttest -c "cd ~/reklamo && scripts/host/install.sh"'
```

**Verified on the live account** with Playwright against reklamo.bg: every public page at iPhone width (no overflow, correct headings, 404 and search templates), then a real order end to end — request form with a 1 MB `.ai` upload, confirmation email with the tracking link, admin login and mockup upload, mockup email with the one-time approval link, approval from the order page, deposit-request email with bank details, admin "approved" email, and the emailed link refusing a second decision. Emails were read from the account's Maildir (`~/mail/reklamo.bg/office/new`) over SSH. Two production-only faults surfaced and were fixed: SMTP port 465 is refused on saturn (587 STARTTLS works), and Exim sender-verifies the envelope address, so WooCommerce's sender must be the `SMTP_USER` mailbox (install sets it; the plugin now forces Return-Path to `SMTP_USER`). The test order, its two files and the test emails were deleted afterwards.
